...
pro qpq6_survey, COVFACT = COVFACT, psfile = psfile, outfile = outfile, NOPS=nops, $
                  CSZ=csz, XMRG=xmrg

  compile_opt strictarr

  ;; Get structure if necessary
  if not keyword_set( PSFILE ) then psfile = 'qpq6_survey.ps'
  ...
  npair = n_elements(qpq6)

  ;; Get angles
  posang, 1, qpq6.rad/15., qpq6.decd, qpq6.rad_bg/15., qpq6.decd_bg, pa
  ...
  for qq=0L,npair-1 do begin
     ...
     instr = qpq6_get_instrument( strtrim(qso_dir+qpq6[qq].lya_fil,2) )
     case instr of
        'SDSS': clbl = 'S'
        'BOSS': clbl = 'B'
        'GMOS': clbl = 'G'
     ...
     if qpq6[qq].z_fg LT 2.5 then cclr = clr.blue else cclr = clr.red

     xpos = qpq6[qq].R_phys * cos(pa[qq]*!pi/180.)
     ypos = qpq6[qq].R_phys * sin(pa[qq]*!pi/180.)
