...
;fig_cgm_map, psfile='qpq_cgm_map_hi.ps',
                                ;range=[0., 10], SIG_FINE=0.05 ; HI
PRO qpq6_lyaew_map, img1, nameplot, RANGE=range, ZOOM=zoom, EPS=eps,$
  ...
  ;; Initialize
  if not keyword_set(PSFILE) then psfile = 'qpq6_lyaew_map.ps'
  ...
  if not keyword_set(SIG_CLM) then sig_clm = 0.10 ; Ang
  if not keyword_set(SIG_FINE) then sig_fine = 0.10 ; dex

  if not keyword_set(SAMP_LIM) then samp_lim = 8L  ;; Minimum number for stats
  ...
  rqpq_fil = '../Analysis/AbsLines/qpq6_random_lya.fits'
  rqpq_strct = xmrdfits(rqpq_fil, 1)
