...
pro fig_fnlya_allz, infil, PSFILE=psfile

  ;; Get structure if necessary
  if not keyword_set( PSFILE ) then psfile = 'fig_fnlya_allz.ps'
  ...
  oplot, lyabin[gdp], log_lyafn[gdp], color=z3clr, psym=2, symsiz=1.5
  xplt = 12.
...
;  xyouts, xplt+0.2, -16.6, 'z~0; Penton+04', color=z0clr, charsiz=lsz
  ...
  oploterror, N_SLLS, fN_SLLS, bin_SLLS, sigfN_SLLS, $
  ...
  oplot, N_LLS, fN_LLS, color=z3clr, linesty=2

  ;; ;;;;
  ;; pLLS (inferred)
  oplot, [N_LLS[0], max(lyabin)], [fN_LLS[0], min(log_lyafn)], $
  ...
  if qq GE 1 then begin
     ...
     oploterror, HI_clm[gd], fn[gd], $ ;replicate(0.05, ngd), $
                 sigfn[gd], color=z0clr, errcolor=z0clr, thick=6, $
                 psym=3
