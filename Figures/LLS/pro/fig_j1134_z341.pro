...
pro fig_j1134_z341, datfil, NTOT=NTOT, CSIZE=csize, PSFILE=psfile, XTINT=xtint, $
...
;endif 
;  
  if not keyword_set(PSFILE) then psfile = 'fig_j1134_z341.ps'
  if not keyword_set( NTOT ) then ntot = 8L
  if not keyword_set( LSIZE ) then lsize = 1.5
  if not keyword_set( LTHICK ) then lthick = 3.
  if not keyword_set( INFIL ) then infil = 'Input/fig_j1134_z341.inp'
  if not keyword_set( DATFIL ) then datfil = '/u/xavier/Keck/HIRES/RedData/J1134+5742/J1134+5742a_f.fits'
