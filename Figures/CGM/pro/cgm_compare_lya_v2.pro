...
;;;    Shows QPQ5 first
pro cgm_compare_lya_v2

  compile_opt strictarr

  ;; Get structure if necessary
  if not keyword_set( PSFILE ) then psfile = 'cgm_compare_lya_v2.ps'
  ...
  for qq=0L,4 do begin
     ...
     ysmpl = 2.7
     scl = 0.25
     if qq GT 0 then xyouts, xsmpl, ysmpl-(1*scl), 'z~2; LBG', color=clr.red, charsi=lsz, align=0.
     if qq GT 1 then xyouts, xsmpl, ysmpl-(2*scl), 'z~0; L*', color=clr.cyan, charsi=lsz, align=0.
     if qq GE 0 then xyouts, xsmpl, ysmpl-(0*scl), 'z~2; QSO', color=clr.yellow, charsi=lsz, align=0.
