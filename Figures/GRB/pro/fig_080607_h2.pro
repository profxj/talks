pro fig_080607_h2, conti, J0_NH2=J0_NH2, J1_NH2=J1_NH2, AV=AV, $
  ...
  if not keyword_set(NOPS) then psfile = 'fig_h2.ps'
...
;  oplot, wv, conti, color=clr.blue, thick=1, linesty=1
  oplot, wv, red_conti, color=clr.red, thick=3, linesty=1
...
;      print, 'NH2 = ', alog10(total(nJ))
  ...
  ;; Data
  oplot, wv, flam, color=clr.black, psym=10, thick=4

  
  if not keyword_set(NOFIT) then begin
      ;; Total model (plot)
      model = x_voigt(rwv, lines, FWHM=model.fwhm)
      oplot, wv, model*red_conti, color=clr.blue, thick=3
  endif
  ...
  if keyword_set(REVIEW) then begin
      ...
      ;; HI
      wlya = 1215.67 * (1+zgrb)
      xyouts, wlya, 0.85, 'HI Ly!9a!X', color=clr.blue, charsiz=2., alignm=0.5
      plotsym, 1, 4., thick=4
      oplot, [wlya], [0.80], psym=8, color=clr.blue
