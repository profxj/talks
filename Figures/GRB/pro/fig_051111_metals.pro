...
pro fig_051111_metals, datfil, NTOT=NTOT, CSIZE=csize, PSFILE=psfile, XTINT=xtint, $
...
;endif 
;  
  if not keyword_set(PSFILE) then psfile = 'fig_051111_metals.ps'
  if not keyword_set( NTOT ) then ntot = 16L
  if not keyword_set( LSIZE ) then lsize = 1.5
  if not keyword_set( LTHICK ) then lthick = 3.
  if not keyword_set( INFIL ) then infil = 'Input/fig_051111_metals.inp'
  if not keyword_set( DATFIL ) then $
    datfil = '/u/xavier/Keck/HIRES/RedData/GRB051111/GRB051111_f.fits'
