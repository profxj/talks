...
pro fig_050730_metals, datfil, NTOT=NTOT, CSIZE=csize, PSFILE=psfile, XTINT=xtint, $
...
;endif 
;  
  if not keyword_set(PSFILE) then psfile = 'fig_050730_metals.ps'
  if not keyword_set( NTOT ) then ntot = 6L
  if not keyword_set( LSIZE ) then lsize = 2.0
  if not keyword_set( LTHICK ) then lthick = 7.
  if not keyword_set( INFIL ) then infil = 'Input/fig_050730_metals.inp'
  if not keyword_set( DATFIL ) then $
    datfil = '/u/xavier/GRB/data/050730/MIKE/FSpec/GRB050730r_f.fits'
...
;if nlin LE ntot then begin
  ...
  if not keyword_set( CSIZE ) then csize = 2.6
...
;      readf, 11, wrest, ymin, ymax, nblnd
      ...
      plot, velo[pmn:pmx], fx[pmn:pmx], xrange=vmnx, $
        ...
        charsize=csize, psym=10, background=clr.white, color=clr.lightgray, $
      ...
      if keyword_set(FINE) then begin 
          if abslin.E NE 0 or strmid(nam,0,4) EQ 'FeII' then $
            xyouts, 0.05*(vmnx[1]-vmnx[0])+vmnx[0], $
                    ymnx[0]+ (ymnx[1]-ymnx[0])*0.10, $
                    strtrim(nam2,2), color=clr.lightgray, charsize=LSIZE
      endif
  ...
  xyouts, 0.04, 0.55, 'Normalized Flux', color=clr.lightgray, $
          alignment=0.5, ORIENTATION=90., /normal, charsize=2.0 
  yps = 0.17
  xyouts, 0.57, yps, 'Relative Velocity (km/s)', color=clr.lightgray, $
