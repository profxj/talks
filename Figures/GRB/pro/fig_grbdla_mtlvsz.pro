...
pro fig_grbdla_mtlvsz, PSFILE=psfile, NBIN=nbin, SURVIVE=SURVIVE, $
              ZMAX=zmax, ZMIN=zmin, NODLA=nodla, LBG=lbg, $
              NARROW=narrow, NONO=nono, XRNG=xrng, BW=bw, TWOPG=twopg, $
              CSZ=csz, LSZ=lsz

  if not keyword_set(PSFILE) then psfile = 'fig_grbdla_mtlvsz.ps'
...
;          oploterror, [grb[ii].zabs], [grb[ii].mtl], [grb[ii].sigmtl], $
  ...
  if keyword_set(LBG) then begin
      ...
      oploterror, [age1], [-0.3], [0.2], color=clr.black, psym=4
      age2 = tot_age-cosm_time(3)/1e9
      oploterror, [age2], [-0.5], [0.2], color=clr.black, psym=4
  endif
...
;  oplot, [4.7], [-2.65], color=clr.green, psym=1
  lsz = 1.8
;  xyouts, xrng[0]-0.2, -2.7, 'QSO-DLA', color=qclr,  charsiz=lsz
;  oplot, [4.7], [-2.40], color=clr.blue, psym=1
  xyouts, xrng[0]-0.2, -2.35, 'GRB-DLA', color=gclr, charsiz=lsz
  if keyword_set(LBG) then $
    xyouts, 4.7, -2.20, 'LBG', color=clr.black, charsiz=lsz
