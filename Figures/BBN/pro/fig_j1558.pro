...
pro fig_j1558, datfil, NTOT=NTOT, CSIZE=csize, PSFILE=psfile, XTINT=xtint, $
...
;readcol, instr_list, instr_fil, inst_dw, inst_w0, format='a,f,f'
  ...
  datfil = ''
  readf, 11, datfil
  datfil = root+'Data/J1558-0031_949fix_f.fits'
  readf, 11, FWHM
...
;if nlin LE ntot then begin
  ...
  plot, wave[pmn:pmx], fx[pmn:pmx], xrange=[4440,4560], $
        yrange=[-0.5,9], xtickn=xspaces, xmargin=[6,2], $
        ymargin=[0,0], xtitl='Wavelength (Ang)', ytit='Relative Flux', $
        charsize=csz2, psym=10, background=clr.white, color=clr.black, $
        xstyle=1, ystyle=1, thick=1, ytickinterval=ytint, $
        pos=[0.1,0.57,0.98,0.83], /nodata
  ...
  restore, mikelyafit, /relaxed

  ;; Shaded region
  lya.N = NHI + sigNHI
  ufx[pmn:pmx] = x_voigt(wave[pmn:pmx], lya, FWHM=FWHM)
  x_curvefill, wave[pmn:pmx], vfx[pmn:pmx]*conti[pmn:pmx], $
               ufx[pmn:pmx]*conti[pmn:pmx], color=sclr
  lya.N = NHI - sigNHI
  lfx[pmn:pmx] = x_voigt(wave[pmn:pmx], lya, FWHM=FWHM)
  x_curvefill, wave[pmn:pmx], vfx[pmn:pmx]*conti[pmn:pmx], $
               lfx[pmn:pmx]*conti[pmn:pmx], color=sclr

  
  ;; Data
  oplot, wave[pmn:pmx], fx[pmn:pmx], color=clr.black, thick=2, psym=10

  ;; Label
  xyouts, 4445., 7., 'HI 1215', color=clr.black, charsize=LSIZE

  ;; PLOT
  oplot, [-10000., 10000.], [0.,0.], color=clr.gray, linestyle=3,thick=1
  oplot, wave, conti, color=clr.blue, linestyle=2, thick=2
  ...
  for ii=0L,nlin-1 do begin
      if ii LT 2 then offy = (npy+3) else offy = (npy-1)
...
;      readf, 11, wrest, ymin, ymax, nblnd
          ...
          oplot, [-10000., 10000.], [0.,0.], color=clr.gray, linestyle=3,thick=4
          oplot, [-10000., 10000.], [1.,1.], color=clr.gray, linestyle=1,thick=3
      ...
      ;; Best Fit
      oplot, velo[pmn:pmx],vfx[pmn:pmx], color=clr.cyan, thick=3
